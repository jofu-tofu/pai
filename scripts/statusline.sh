...
#
# Modified for cross-platform compatibility (macOS, Linux, Windows/Git Bash)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────────────────────────────────────────
# On Windows, jq may not be in PATH. Find and add it.

if ! command -v jq >/dev/null 2>&1; then
    # Try common Windows jq locations
    JQ_PATHS=(
    ...
    for jq_path in "${JQ_PATHS[@]}"; do
        if [ -x "$jq_path/jq.exe" ] || [ -x "$jq_path/jq" ]; then
...
detect_terminal_width() {
    ...
    # Tier 2: Direct TTY query (skip on Windows where /dev/tty doesn't work)
...
fetch_location() {
    ...
    [ -f "$LOCATION_CACHE" ] && cache_age=$(($(date +%s) - $(get_file_mtime "$LOCATION_CACHE" || echo 0)))
...
fetch_weather() {
    local cache_age=999999
    [ -f "$WEATHER_CACHE" ] && cache_age=$(($(date +%s) - $(get_file_mtime "$WEATHER_CACHE" || echo 0)))
...
if [ "$MODE" = "normal" ]; then
    ...
    quote_age=$(($(date +%s) - $(get_file_mtime "$QUOTE_CACHE" || echo 0)))
