...
```bash
mkdir -p $PAI_DIR/skills/Evals/UseCases/<name>/{test-cases,golden-outputs,prompts}
```

### Step 3: Create Config File

Create `$PAI_DIR/skills/Evals/UseCases/<name>/config.yaml`:
...
Create `$PAI_DIR/skills/Evals/UseCases/<name>/prompts/v1.0.0.md`:
...
Create test cases in `$PAI_DIR/skills/Evals/UseCases/<name>/test-cases/`:
...
Create `$PAI_DIR/skills/Evals/UseCases/<name>/README.md`:
...
\`\`\`bash
bun run $PAI_DIR/skills/Evals/EvalServer/cli-run.ts --use-case <name>
\`\`\`
...
```bash
# Check structure
ls -la $PAI_DIR/skills/Evals/UseCases/<name>/

# Validate config
bun run $PAI_DIR/skills/Evals/EvalServer/cli.ts use-case show <name>
```
...
bun run $PAI_DIR/skills/Evals/EvalServer/cli-run.ts \
  --use-case <name> \
  --test-id 001-basic \
  --verbose
```
