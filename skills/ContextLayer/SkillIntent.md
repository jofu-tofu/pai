...
4. **Falsifiability-enforced:** Every line in every CLAUDE.md passes the falsifiability test: removing it would cause an agent to fail or produce wrong behavior. No line exists just to be thorough or to look comprehensive.
...
3. **Context Maintenance template embedded in every generated file** — Self-sustainability depends on this template being present. Removing it from generated output breaks the self-sustaining property.
4. **Workflows remain cost-tiered** — Prune must not introduce filesystem reads. Generate is the expensive operation. This cost ordering must not invert.
