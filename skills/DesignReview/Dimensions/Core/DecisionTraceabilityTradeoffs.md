...
- **C17 (Declared depth target)** — Decision detail should match intended depth.
...
2. Alternatives and tradeoffs are documented for significant decisions that materially affect behavior, risk, or scope.
3. Consequences of decisions are described at a depth appropriate to C17.
4. If decision documentation is expected given the design type but absent, note it — grounding the expectation in context.
5. Do not require alternatives for every minor decision; focus on high-impact decisions.
