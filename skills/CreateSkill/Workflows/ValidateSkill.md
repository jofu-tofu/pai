...
Verify TitleCase:
- PASS: `Blogging`, `Daemon`, `CreateSkill`
...
Verify TitleCase:
- PASS: `ManageServer.ts`, `ManageServer.help.md`
...
```bash
bun $PAI_DIR/skills/[SkillName]/Tools/[ToolName].ts --help
```
...
```bash
grep -l "Intent-to-Flag" $PAI_DIR/skills/[SkillName]/Workflows/*.md
```
