...
1. **Send voice notification**:
   ```bash
   curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d "{\"message\": \"Running the WORKFLOWNAME workflow in the Browser skill to ACTION\"}" > nul 2>&1
   ```
