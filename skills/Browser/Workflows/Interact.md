...
```bash
curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d "{\"message\": \"Running the Interact workflow in the Browser skill to interact with elements\"}" > nul 2>&1
```
