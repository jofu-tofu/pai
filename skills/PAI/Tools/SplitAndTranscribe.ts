...
async function splitAudioFile(
  ...
  // Cross-platform: use tmpdir() instead of hardcoded /tmp
  ...
  return new Promise((resolve, reject) => {
    // Cross-platform: use getWindowsShellOptions() for Windows compatibility
...
if (import.meta.main) {
  ...
  if (!filePath) {
    console.error("Usage: bun SplitAndTranscribe.ts <file> [format]");
    process.exit(1);
  }
