...
function loadVariables(): Record<string, string> {
  try {
    ...
    console.warn("⚠️ Could not read settings.json, using defaults:", e);
    return getDefaultVariables();
  }
}
...
function loadAlgorithm(): string {
  ...
  if (!existsSync(latestFile)) {
    throw new Error(`Algorithm LATEST file not found: ${latestFile}`);
  }
  ...
  if (!existsSync(algorithmFile)) {
    throw new Error(`Algorithm version file not found: ${algorithmFile}`);
  }
