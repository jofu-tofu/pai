...
```bash
bun run skills/CORE/Tools/SessionProgress.ts <command>
```
...
```bash
bun run skills/CORE/Tools/FeatureRegistry.ts <command>
```
...
**Based on Anthropic's 6-step startup:**
...
**Automatic!** The startup hook checks for active work and displays it at session start:
...
Manual check: `bun run skills/CORE/Tools/SessionProgress.ts list`

### Step 4: Review Last Session State

```bash
bun run skills/CORE/Tools/SessionProgress.ts resume <project>
```
...
bun run skills/CORE/Tools/SessionProgress.ts create my-feature \
  "Implement user authentication" \
  "Add OAuth providers"

# 2. Initialize feature registry (if multiple features)
bun run skills/CORE/Tools/FeatureRegistry.ts init my-feature
bun run skills/CORE/Tools/FeatureRegistry.ts add my-feature "Login form" --priority P1
bun run skills/CORE/Tools/FeatureRegistry.ts add my-feature "Password reset" --priority P2

# 3. Record progress as you work
bun run skills/CORE/Tools/SessionProgress.ts decision my-feature \
  "Using JWT tokens" "Simpler than sessions for API-first architecture"

bun run skills/CORE/Tools/SessionProgress.ts work my-feature \
  "Created User model and migration" src/models/user.ts src/migrations/001_users.sql

# 4. Before ending session, set handoff
bun run skills/CORE/Tools/SessionProgress.ts next my-feature \
  "Write auth service tests" "Implement login endpoint" "Add JWT validation"

bun run skills/CORE/Tools/SessionProgress.ts handoff my-feature \
...
bun run skills/CORE/Tools/SessionProgress.ts list

# 2. Load context for specific project
bun run skills/CORE/Tools/SessionProgress.ts resume my-feature

# 3. Check feature status
bun run skills/CORE/Tools/FeatureRegistry.ts list my-feature

# 4. Get next feature to work on
bun run skills/CORE/Tools/FeatureRegistry.ts next my-feature
```
