// C:\Users\fujos\.pai\observability\apps\server\src\file-ingest.ts
// File-based event streaming - watches JSONL files
// Windows-compatible version using polling for file watching
...
// Polling intervals for file watching (Windows-compatible)
...
function loadAgentSessions(): void {
  ...
  if (!existsSync(sessionsFile)) {
    console.log('agent-sessions.json not found, agent names will use defaults');
    return;
  }
...
// Windows-compatible file watcher using polling
function watchFilePolling(filePath: string): void {
  ...
  const intervalId = setInterval(() => {
    try {
      ...
      console.error(`Error polling ${filePath}:`, error);
    }
  }, POLL_INTERVAL);

  fileWatchers.set(filePath, intervalId);
}

function watchAgentSessionsPolling(): void {
  const paiDir = process.env.PAI_DIR || join(homedir(), '.pai');
  const sessionsFile = join(paiDir, 'agent-sessions.json');
  ...
  let lastModified = statSync(sessionsFile).mtimeMs;

  setInterval(() => {
    try {
      if (!existsSync(sessionsFile)) return;

      const currentModified = statSync(sessionsFile).mtimeMs;
...
export function startFileIngestion(callback?: (events: HookEvent[]) => void): void {
  ...
  loadAgentSessions();
  watchAgentSessionsPolling();

  const todayFile = getTodayEventsFile();
  watchFilePolling(todayFile);
...
export function stopFileIngestion(): void {
  for (const [filePath, intervalId] of fileWatchers.entries()) {
    clearInterval(intervalId);
    watchedFiles.delete(filePath);
  }
  fileWatchers.clear();
  console.log('File ingestion stopped');
}
