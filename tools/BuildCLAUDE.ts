...
const ALGORITHM_DIR = join(PAI_DIR, "skills/PAI/Algorithm");
...
function getAlgorithmVersion(): string {
  if (!existsSync(LATEST_PATH)) {
    console.error("⚠ skills/PAI/Algorithm/LATEST not found, defaulting to v3.5.0");
    return "v3.5.0";
  }
...
function loadVariables(): Record<string, string> {
  ...
  return {
    ...
    "{{ALGO_PATH}}": `skills/PAI/Algorithm/${algoVersion}.md`,
  };
}
...
export function needsRebuild(): boolean {
  ...
  const algoVersion = getAlgorithmVersion();
  const algoPathPattern = /skills\/PAI\/Algorithm\/(.+?)\.md/;
