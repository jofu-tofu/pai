...
function determineSessionFocus(filesChanged: string[], commandsExecuted: string[]): string {
  ...
  if (filePatterns.some(f => f.includes('/blog/') || f.includes('\\blog\\') || f.includes('/posts/') || f.includes('\\posts\\'))) return 'blog-work';
  if (filePatterns.some(f => f.includes('/hooks/') || f.includes('\\hooks\\'))) return 'hook-development';
  if (filePatterns.some(f => f.includes('/skills/') || f.includes('\\skills\\'))) return 'skill-updates';
  if (filePatterns.some(f => f.includes('/agents/') || f.includes('\\agents\\'))) return 'agent-work';
